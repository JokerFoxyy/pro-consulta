...
namespace ProConsulta.Repositories.Agendamentos
{
    public class AgendamentoRepository : IAgendamentoRepository
    {
        ...
        {
            return await _context.Agendamentos.Include(x=>x.Paciente).Include(x=>x.Medico).AsNoTracking().ToListAsync();
        }

        public Task<Agendamento?> GetByIdAsync(int id)
        {
            return _context.Agendamentos.Include(x => x.Paciente).Include(x => x.Medico).SingleOrDefaultAsync(o => o.Id == id);
        }
    }
}
